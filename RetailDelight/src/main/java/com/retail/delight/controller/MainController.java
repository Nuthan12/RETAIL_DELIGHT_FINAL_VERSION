...
public class MainController {
	...
	public String listProductHandler(Model model, //
		...
		PaginationResult<ProductInfo> result = productService.productList(likeName, page);
	...
	public String listProductHandler(HttpServletRequest request, Model model, //
		...
		if (code != null && code.length() > 0) {
			...
			logger.debug(
					"The product {} with code {} to be added to the cart is currently been fetched from the product database",
		...
		if (product != null) {
			...
			logger.debug(
					"The product {} with code {} to be added to the cart has been succesfully fetched from the product database",
					productInfo.getName(), productInfo.getCode());
	...
	public String removeProductHandler(HttpServletRequest request, Model model, //
		...
		if (code != null && code.length() > 0) {
			product = productService.getProduct(code);
			logger.debug(
					"The product {} with code {} to be removed from the cart is currently been fetched from the product database",
					product.getName(), product.getCode());
		}
		if (product != null) {
			CartInfo cartInfo = Utils.getCartInSession(request);
			ProductInfo productInfo = new ProductInfo(product);
			logger.debug(
					"The product {} with code {} to be removed from the cart has been succesfully fetched from the product database",
					productInfo.getName(), productInfo.getCode());
	...
	public String orderList(Model model, //
		...
		PaginationResult<OrderInfo> paginationResult = orderService.billListPagination(pageStr);
	...
	public String orderView(Model model, @RequestParam("orderId") String orderId) {
		...
		if (orderInfo == null) {
			logger.error("Fetching the order details for the bill from the database for the order id {} not possible",
					orderId);
